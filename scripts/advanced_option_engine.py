...
class NVIICharacteristics:
    ...
    dividend_yield: float = 0.063
    target_leverage: float = 1.25
    leverage_range: Tuple[float, float] = (1.05, 1.50)
    expense_ratio: float = 0.0095
    daily_rebalancing_cost: float = 0.0001
...
class AdvancedCoveredCallEngine:
    ...
    def calculate_net_option_premium(self, gross_premium: float, 
        ...
        total_proceeds = net_premium * contracts * 100 - commission
        return total_proceeds / (contracts * 100)
    def analyze_covered_call_strategy(self, 
        ...
        max_loss_scenario = -self.nvii.current_price * 0.5
        return {
            'regime': regime.value,
