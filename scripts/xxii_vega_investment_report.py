            ...
            for symbol in all_symbols:
                try:
                    ...
                    if not hist.empty:
                        ...
                        market_cap = info.get('marketCap', current_price * 100_000_000)
    ...
    def generate_investment_analysis(self, vega_df):
        ...
        for symbol in [self.symbol] + self.comparison_symbols:
            ...
            if not symbol_atm.empty:
                ...
                vega_score = row['vega'] / atm_30d['vega'].max() * 100
                ...
                cap_score = 100 - (np.log10(row['market_cap']) - 6) * 10
                cap_score = max(0, min(100, cap_score))
                beta_score = min(100, row['beta'] * 25)
