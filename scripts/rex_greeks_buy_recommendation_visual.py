...
class REXGreeksBuyRecommendation:
    ...
    def calculate_theoretical_scores(self):
        ...
        for etf, data in self.greeks_data.items():
            vega_score = data['vega_correlation'] * 10
            momentum_score = max(data['call_price_correlation'], 0) * 5
            stability_score = min(data['factor_r_squared'] / 10, 3)
            yield_score = min(data['option_yield'], 4)
...
Mathematical expectation: Outperformance during vol expansion AND efficient premium capture during vol contraction.
        ...
        for etf in etf_list:
            ...
            row = [
                ...
                max(data['call_price_correlation'], 0),
                data['avg_vega'],
                data['option_yield'] / 100,
                data['factor_r_squared'] / 100
            ]
